async function fetchTrendingTags() {
  try {
    ...
    const response = await fetch('https://mastodon.online/api/v1/trends/tags');
    if (!response.ok) {
...
function displayTrendingTags(tags) {
  ...
  topTags.forEach(tag => {
    ...
    listItem.innerHTML = `<a href="https://mastodon.online/tags/${tag.name}" target="_blank">#${tag.name}</a> - ${tag.history[0].uses} uses`;
